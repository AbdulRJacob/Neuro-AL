...
class SHAPES_4:
    ...
    def check_position(self,obj_pos,postions,idx=0):
        ...
        if cur_pos == Position.LEFT:
            if obj_pos[obj_2] in [0,2]:
                return False
            elif obj_pos[obj_2] in [1,3] and obj_pos[obj_1] in [0,2]:
                return True and self.check_position(obj_pos,postions,idx+1)
            
        if cur_pos  == Position.ABOVE:
            if obj_pos[obj_2] in [0,1]:
                return False 
            elif obj_pos[obj_2] in [2,3] and obj_pos[obj_1] in [0,1]:
                return True and self.check_position(obj_pos,postions,idx+1)
    ...
    def find_section(self,x, y):
        ...
        section_index = section_y * 2 + section_x
...
class SHAPESDATASET(Dataset):
    def __init__(
        self,
        data_dir: str = "/mnt/d/fyp/SHAPES_4/training_data/",
        transform=None,
    ...
    def _load_image(self, sample):
        ...
        if len(labels_arr) == 0:
            return  image_path , np.zeros((self.num_slots,9))
