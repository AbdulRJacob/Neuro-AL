...
# Function to seed all random number generators
...
# Load datasets
datasets = {
    split: CLEVR(data_dir=data_dir, transform=aug[split], cache=True)
...
# Load dataloaders
dataloaders = {
    split: DataLoader(datasets[split], shuffle=(split == "train"), drop_last=(split == "train"), **kwargs)
...
def objective(trial):
    ...
    # Update batch size in dataloaders
